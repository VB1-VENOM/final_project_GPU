...
echo "N,CPU_Time,GPU_Time,MPI_Time,CPU_GFLOPs,GPU_GFLOPs,MPI_GFLOPs,MPI_SUMMA_Time,MPI_SUMMA_GFLOPs" > results.csv
...
do
    ...
    # ---------------- MPI + GPU + SUMMA ----------------
    MPI_SUMMA_OUT=$(mpirun -np 4 ./main $V)
    MPI_SUMMA_TIME=$(echo "$MPI_SUMMA_OUT" | grep "time" | awk '{print $10}')
    if [ -n "$MPI_SUMMA_TIME" ]; then
        MPI_SUMMA_GFLOPs=$(echo "scale=6; 2 * $V * $V * $V / ($MPI_SUMMA_TIME * 1000000000)" | bc)
    else
        MPI_SUMMA_GFLOPs=""
    fi
    ...
    # ---------------- SAVE TO CSV ----------------
    echo "$V,$CPU_TIME,$GPU_TIME,$MPI_TIME,$CPU_GFLOPs,$GPU_GFLOPs,$MPI_GFLOPs,$MPI_SUMMA_TIME,$MPI_SUMMA_GFLOPs" >> results.csv
done
