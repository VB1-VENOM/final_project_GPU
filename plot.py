...
mpi_summa_gflops = df["MPI_SUMMA_GFLOPs"]

cpu_time = df["CPU_Time"]
gpu_time = df["GPU_Time"]
mpi_time = df["MPI_Time"]
mpi_summa_time = df["MPI_SUMMA_Time"]
...
plt.plot(N, mpi_summa_gflops, label="MPI SUMMA + GPU")
...
plt.plot(N, mpi_summa_time, label="MPI SUMMA + GPU")
...
plt.scatter(intensity, mpi_summa_gflops, label="MPI SUMMA + GPU", marker="^")
